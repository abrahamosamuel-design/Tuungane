...
function NewRequest() {
  ...
  const runSuggest = async () => {
    ...
    setSuggesting(true);
    try {
      // TODO: Re-implement suggestCategory in Node.js backend
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data: p } = await apiClient<{ data: any }>(`/profiles/public/${search.profileId}`);
        if (!p) return;
        ...
        if (search.serviceId) {
          const { data: services } = await apiClient<{ data: any[] }>(`/services/profile/${search.profileId}`);
          const svc = services?.find(s => s.id === search.serviceId);
          if (svc) {
        ...
        console.error("Failed to fetch target profile or service", err);
      }
    })();
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const { data: inserted } = await apiClient<{ data: { id: string } }>(`/requests`, {
        method: 'POST',
        body: JSON.stringify({

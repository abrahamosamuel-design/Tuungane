...
function Dashboard() {
  ...
  const load = async () => {
    if (!user) return;
    try {
      const { data } = await apiClient<{ data: any }>("/dashboard/stats");
      const { profile: p, serviceProfile: s, posts: ps, providerStats, customerStats: cs } = data.data;
      ...
      <section className="mx-auto max-w-3xl px-4 py-8 pb-40 [padding-bottom:calc(10rem+env(safe-area-inset-bottom))]">
        ...
        <div className="mt-6">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <QuickAction to="/profiles/new" icon={<Wrench className="h-5 w-5" />} label="Add a service" hint="Each service gets its own public page" />
...
function ServiceProfileForm({ onSaved }: { onSaved: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      await apiClient.post("/dashboard/service-profile", {
        business_name: businessName || null, category_slug: categorySlug, subcategory, bio, district, town, phone: phone || null, whatsapp: whatsapp || null,
      });

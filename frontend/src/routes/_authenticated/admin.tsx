...
function Admin() {
  ...
  if (!isModerator) {
    return <><div className="mx-auto max-w-2xl px-4 py-16 text-center text-muted-foreground">Moderator access required. {isAdmin ? "" : "Ask an admin to grant you moderator rights via the user_roles table."}</div></>;
  }
...
function UsersTab() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await apiClient<{ data: { users: any[], contacts: Record<string, any>, roles: Record<string, ManageableRole[]> } }>("/admin/users");
        setRows(data.data.users);
        setContacts(data.data.contacts);
        if (isAdmin) setRoles(data.data.roles);
      } catch (err) {
        console.error(err);
      }
    })();
  ...
  const toggleRole = async (userId: string, role: ManageableRole, has: boolean) => {
    ...
    setBusy(`${userId}:${role}`);
    try {
      await apiClient.post(`/admin/users/${userId}/role/${role}`, { has });
      setRoles((prev) => {
...
function ProvidersTab() {
  ...
  const load = async () => {
    try {
      const { data } = await apiClient<{ data: any[] }>("/admin/providers");
  ...
  const setVerified = async (uid: string, v: "verified" | "featured" | "none") => {
    try {
      await apiClient.put(`/admin/providers/${uid}/status`, { verified: v });
      toast.success("Updated"); load();
    } catch (err) { toast.error("Failed to update"); }
  };
  const toggleSuspend = async (uid: string, current: boolean) => {
    try {
      await apiClient.put(`/admin/providers/${uid}/status`, { suspended: !current });
...
function PostsTab() {
  ...
  const load = async () => {
    try {
      const { data } = await apiClient<{ data: PostRow[] }>("/admin/posts");
      setPosts(data.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const featurePost = async (id: string, current: boolean) => {
    try {
      await apiClient.put(`/admin/posts/${id}/feature`, { featured: !current });
      load();
    } catch (err) { toast.error("Failed to feature"); }
  };
  return (
...
function RecsTab() {
  ...
  const load = async () => {
    try {
      const { data } = await apiClient<{ data: any[] }>("/admin/recs");
      setRecs(data.data);
  ...
  const hideRec = async (id: string, hidden: boolean) => {
    try {
      await apiClient.put(`/admin/recs/${id}/hide`, { hidden: !hidden });
      load();
...
function ReportsTab() {
  ...
  const load = async () => {
    try {
      const { data } = await apiClient<{ data: any[] }>(`/admin/reports?status=${filter}`);
...
function OfficialTabContent({ initialSub }: { initialSub?: SubTab }) {
  ...
  const load = async () => {
    try {
      const { data } = await apiClient<{ data: any }>("/admin/official");
      setAccount(data.data.account);
      setPosts(data.data.posts);
      setSeeded(data.data.seeded);
  ...
  const revealClaimContact = async (id: string) => {
    ...
    try {
      const { data } = await apiClient<{ data: any }>(`/admin/official/claim-contact/${id}`);
      if (data.data) setRevealed((prev) => ({ ...prev, [id]: data.data }));
    } catch (err) { toast.error("Could not reveal contact"); }
  };
  useEffect(() => { load(); }, []);

  const togglePost = async (id: string, field: "is_featured" | "is_pinned" | "is_homepage", v: boolean) => {
    try { await apiClient.put(`/admin/official/posts/${id}`, { field, value: !v }); load(); } catch (err) { toast.error("Failed"); }
  };
  const deletePost = async (id: string) => {
    if (!confirm("Delete this official post?")) return;
    try { await apiClient.delete(`/admin/official/posts/${id}`); load(); } catch (err) { toast.error("Failed"); }
  };
  const setSeededStatus = async (uid: string, status: "unclaimed" | "claim_pending" | "claimed") => {
    try { await apiClient.put(`/admin/official/seeded/${uid}/status`, { seeded_status: status }); load(); } catch (err) { toast.error("Failed"); }
  };
  const reviewClaim = async (id: string, requester: string, profileUid: string, decision: "approved" | "rejected") => {
    try {
      await apiClient.put(`/admin/official/claims/${id}/review`, { requester_user_id: requester, service_profile_user_id: profileUid, decision });
      toast.success(decision === "approved" ? "Claim approved and profile reassigned" : "Claim rejected");
      load();
    } catch (err) { toast.error("Failed"); }
  };

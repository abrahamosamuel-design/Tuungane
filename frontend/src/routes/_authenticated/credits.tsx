...
function CreditsPage() {
  ...
  const [realtimeStatus, setRealtimeStatus] = useState<string>("polling");
  ...
  const loadPersonal = async () => {
    if (!user) return;
    try {
      const { data } = await apiClient<{ data: { transactions: Tx[]; requests: Req[] } }>("/credits/personal");
      setTxs(data.data?.transactions ?? []);
      setReqs(data.data?.requests ?? []);
      setLastEventAt(new Date().toISOString());
      setLastEventKind("poll_success");
    } catch (err) {
      console.error(err);
      setLastEventAt(new Date().toISOString());
      setLastEventKind("poll_error");
    }
  };
  ...
  // Realtime (replaced with polling for now to avoid supabase client dependency)
  useEffect(() => {
    if (!user) { setRealtimeStatus("idle"); return; }
    const interval = setInterval(loadPersonal, 10000);
    setRealtimeStatus("polling_10s");
  ...
  const checks: Check[] = isLoggedIn ? [
    ...
    { label: "Realtime channel SUBSCRIBED", pass: realtimeStatus === "polling_10s", detail: `status=${realtimeStatus}` },

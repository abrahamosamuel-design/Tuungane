...
function CategoryPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await apiClient.get(`/services/category/${category.slug}`);
        if (res.data) {
          setList(res.data.list as Row[]);
          setOthers(res.data.others as Row[]);
        }
      } catch (err) {
  ...
  const allProviderIds = useMemo(() => {
    ...
    (filtered ?? []).forEach(p => ids.add(p.user_id));
    (others ?? []).forEach(p => ids.add(p.user_id));
            ...
            <>
              ...
              {filtered.length > 0 ? (
                <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                  {filtered.map((p) => <RealProviderCard key={p.user_id} p={p} userLoc={userLoc} prefetchedTrustLevel={trustBadges?.get(p.user_id)} />)}
                </div>
              ) : (
                !radiusExpanded && (
                    ...
                    {others.length > 0 && (
                      <div>
                        ...
                        <div className="mt-4 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                          {others.map((p) => <RealProviderCard key={p.user_id} p={p} userLoc={userLoc} prefetchedTrustLevel={trustBadges?.get(p.user_id)} />)}
                        </div>
                      </div>
                    )}
                  </div>
...
function RealProviderCard({ p, userLoc, prefetchedTrustLevel }: { p: Row; userLoc: UserLocation | null; prefetchedTrustLevel?: TrustLevel | null; }) {

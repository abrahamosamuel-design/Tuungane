...
export async function fetchFeaturedLocations(force = false): Promise<FeaturedLocation[]> {
  ...
  try {
    const { data: res } = await apiClient("/services/featured-locations");
    _cache = res.data ?? [];
    _cacheAt = Date.now();
    return _cache!;
  } catch (err) {
...
// (Auth state change listeners are removed for apiClient migration, they will be handled upstream)

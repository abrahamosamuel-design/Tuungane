...
export function useBoostedSet(entityType: string, boostTypes: BoostType[]) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const { data } = await apiClient(`/credits/boosted-set?entity_type=${entityType}&types=${boostTypes.join(",")}`);
        if (cancelled) return;
        setIds(new Set(((data?.data ?? []) as Array<{ entity_id: string }>).map((b) => b.entity_id)));
      } catch (err) {
        console.error("Failed to fetch boosted set", err);
      } finally {
        setLoaded(true);
      }
    })();

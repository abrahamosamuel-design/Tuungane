...
export function MatchingRequestsSection() {
  ...
  const fetchFresh = useCallback(async () => {
    if (!user) return;
    try {
      const res = await apiClient.get("/requests/me/matching");
      if (res.data) {
        const rows = res.data.data as ServiceRequestRow[];
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/requests/browse" className="inline-flex items-center gap-1 text-xs font-semibold text-orange hover:underline">Open feed <ArrowRight className="h-3 w-3" /></Link>
        </div>
      </div>

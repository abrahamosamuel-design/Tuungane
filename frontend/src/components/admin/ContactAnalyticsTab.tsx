...
export function ContactAnalyticsTab() {
  ...
  const load = async () => {
    try {
      ...
      const { data } = await apiClient<{ data: { logs: any[], reveals: RevealRow[], profiles: { id: string, full_name: string }[], settings: { id: string, setting_value: any } | null } }>(`/admin/contact-analytics?${q.toString()}`);
      
      const raw = data.data.logs;
      ...
      setLogs(ll);
      setReveals(data.data.reveals);
      setProfiles(new Map((data.data.profiles).map((p) => [p.id, p.full_name || p.id.slice(0, 8)])));
      
      const s = data.data.settings;
      const sv = (s?.setting_value as VisibilitySettings | undefined) ?? {};
  ...
  const saveSettings = async () => {
    setBusy(true);
    try {
      await apiClient.post("/admin/contact-analytics/settings", { settingsId, settings });
      toast.success("Contact visibility settings saved"); load();

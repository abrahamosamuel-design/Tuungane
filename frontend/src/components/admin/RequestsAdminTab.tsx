...
export function RequestsAdminTab() {
  ...
  const load = async () => {
    try {
      const { data } = await apiClient<{ data: { requests: ServiceRequestRow[], feedback: ServiceFeedbackRow[], disputes: any[] } }>("/admin/requests");
  ...
  const setStatus = async (id: string, status: RequestStatusValue) => {
    try {
      await apiClient.put(`/admin/requests/${id}/status`, { status });
      toast.success("Updated"); load();
    } catch (err) { toast.error("Failed"); }
  };
  const hideReview = async (id: string, v: boolean) => {
    try {
      await apiClient.put(`/admin/requests/feedback/${id}/hide`, { is_visible: !v });
  ...
  const deleteReview = async (id: string) => {
    ...
    try {
      await apiClient.delete(`/admin/requests/feedback/${id}`);
      load();
    } catch (err) { toast.error("Failed"); }
  };
  const resolveDispute = async (id: string, status: "resolved" | "dismissed") => {
    try {
      await apiClient.put(`/admin/requests/disputes/${id}/resolve`, { status });

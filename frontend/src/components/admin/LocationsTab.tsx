...
export function LocationsTab() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const { data } = await apiClient<{ data: { locs: FeaturedLocation[], setting: ExpansionSettings | null } }>("/admin/locations");
      setRows(data.data.locs || []);
      if (data.data.setting) {
        setSettings({ ...DEFAULT_EXPANSION, ...data.data.setting });
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const toggleActive = async (row: FeaturedLocation) => {
    try {
      await apiClient.put(`/admin/locations/${row.id}`, { active: !row.active });
    } catch (error: any) {
  ...
  const updatePriority = async (row: FeaturedLocation, priority: number) => {
    try {
      await apiClient.put(`/admin/locations/${row.id}`, { priority });
    } catch (error: any) {
  ...
  const saveSettings = async () => {
    ...
    try {
      await apiClient.post("/admin/locations/settings", clean);
    } catch (error: any) {

  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        let qs = "";
        if (hasCoords) {
          qs = `?latitude=${userLoc.latitude}&longitude=${userLoc.longitude}`;
        }
        const { data: res } = await apiClient(`/services/home-nearby${qs}`);
        if (!cancelled && res) {
          setRequests(res.requests || []);
          setProviders(res.providers || []);
        }
      } catch (err) {
        console.error("Failed to fetch nearby home section", err);
        if (!cancelled) {
          setRequests([]);
          setProviders([]);
        }

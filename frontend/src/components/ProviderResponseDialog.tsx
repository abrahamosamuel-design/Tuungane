...
export function ProviderResponseDialog({ open, onClose, requestId, existing, onSaved }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      if (existing) {
        await apiClient.patch(`/requests/responses/${existing.id}`, payload);
      } else {
        await apiClient.post(`/requests/${requestId}/responses`, payload);
      }
      ...
      toastError(err, "Couldn't send your response");
    } finally {
      setBusy(false);
    }
  };
  ...
  const withdraw = async () => {
    if (!existing) return;
    try {
      await apiClient.patch(`/requests/responses/${existing.id}`, { status: "withdrawn" });
      toast.success("Response withdrawn");
      onSaved?.();
      onClose();
    } catch (err: any) {
      toastError(err, "Couldn't withdraw response");
    } finally {
      setBusy(false);
    }
  };

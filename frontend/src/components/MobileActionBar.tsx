 ...
 */
export function MobileActionBar({ children, className = "bottom-16" }: { children: ReactNode; className?: string }) {
  return (
    <>
      {/* Spacer so page content isn't covered when scrolled to bottom */}
      <div className="h-24 md:hidden" aria-hidden />
      <div className={`fixed inset-x-0 z-30 border-t border-border bg-background/95 pb-[env(safe-area-inset-bottom)] backdrop-blur md:hidden ${className}`}>

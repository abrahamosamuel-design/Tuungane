...
export function SaveButton({ providerUserId, variant = "icon" }: { providerUserId: string; variant?: "icon" | "full" }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data } = await apiClient<{ data: { saved: boolean } }>(`/social/saved_providers/${providerUserId}`);
  ...
  const toggle = async () => {
    ...
    try {
      const { saved: newStatus } = await apiClient<{ saved: boolean }>(`/social/saved_providers/toggle`, {
        method: 'POST',
      ...
      toast.error(err instanceof Error ? err.message : "Failed to toggle saved status");
    } finally {
      setBusy(false);
    }
  };

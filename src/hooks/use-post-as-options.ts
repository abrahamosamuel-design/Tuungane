  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [{ data: me }, { data: sp }, { data: bp }] = await Promise.all([
        ...
        supabase.from("service_profiles").select("user_id,business_name,cover_url").eq("user_id", userId).maybeSingle(),
        supabase.from("business_pages").select("id,name,logo_url").eq("owner_id", userId),
      ]);
      if (cancelled) return;

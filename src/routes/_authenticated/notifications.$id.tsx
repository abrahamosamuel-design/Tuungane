...
function NotificationDetailPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!active) return;
      if (error || !n) { setBusy(false); setNotif(null); return; }

      let actor: { full_name: string; avatar_url: string | null } | undefined;
      if (n.actor_id) {
        const { data: prof } = await supabase.from("profiles").select("id,full_name,avatar_url").eq("id", n.actor_id).maybeSingle();
        if (prof) actor = prof;
      }

      setNotif({ ...n, actor } as Notif);
      setBusy(false);

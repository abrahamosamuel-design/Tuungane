...
function OfficialPage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data: a } = await supabase.from("official_accounts").select("*").order("created_at").limit(1).maybeSingle();
      setAccount(a as OfficialAccountRow | null);
      const { data: ps } = await supabase.from("official_posts").select("*").eq("status", "published").order("is_pinned", { ascending: false }).order("created_at", { ascending: false });
      setPosts((ps ?? []) as OfficialPostRow[]);
      setLoading(false);
    })();
  }, []);

...
export function RequestsAdminTab() {
  ...
  const load = async () => {
    const [r, f, d] = await Promise.all([
      // admin_list_service_requests is SECURITY DEFINER + admin-gated;
      // returns full rows including revoked contact columns.
      supabase.rpc("admin_list_service_requests", { _limit: 200 }),
    ...
    setRequests(((r.data ?? []).map((x: Record<string, unknown>) => ({ ...x, completion_code: null }))) as ServiceRequestRow[]);

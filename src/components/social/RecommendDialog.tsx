...
export function RecommendDialog({ open, onClose, providerUserId }: { open: boolean; onClose: () => void; providerUserId: string }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    const { error } = await supabase.from("provider_recommendations").upsert({
      provider_user_id: providerUserId, user_id: user.id, service: service.trim(), message: message.trim(), rating,
    }, { onConflict: "provider_user_id,user_id" });
    setBusy(false);
    if (error) toast.error(error.message);
    else { toast.success("Recommendation posted. Thank you!"); onClose(); setService(""); setMessage(""); }
  };
